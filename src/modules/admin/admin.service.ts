...
class AdminService {
  ...
  async getDashboard() {
    ...
    const [
      ...
      Commission.aggregate([{ $group: { _id: null, total: { $sum: '$netAmount' } } }]),
      Commission.aggregate([{ $match: todayFilter }, { $group: { _id: null, total: { $sum: '$netAmount' } } }]),
      ...
      MultiLevelReward.aggregate([{ $group: { _id: null, total: { $sum: '$netAmount' } } }]),
      MultiLevelReward.aggregate([{ $match: todayFilter }, { $group: { _id: null, total: { $sum: '$netAmount' } } }]),
      RankReward.aggregate([{ $group: { _id: null, total: { $sum: '$netAmount' } } }]),
      RankReward.aggregate([{ $match: todayFilter }, { $group: { _id: null, total: { $sum: '$netAmount' } } }]),
